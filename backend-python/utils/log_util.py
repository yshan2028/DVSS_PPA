...
class LogUtil:
    ...
    @classmethod
    def log_response(
        cls, method: str, url: str, status_code: int, execution_time: float, user_id: Optional[int] = None
    ):
        """
        记录响应日志
    ...
    @classmethod
    def log_security(
        cls, event: str, user_id: Optional[int] = None, ip: Optional[str] = None, details: Optional[str] = None
    ):
        """
        记录安全日志
    ...
    @classmethod
    def log_error(cls, error: Exception, context: Optional[str] = None, user_id: Optional[int] = None):
        """
        记录错误日志
        ...
        logger.error(
            f'ERROR - {type(error).__name__}: {str(error)} - Context:{context} - User:{user_id}', exc_info=True
        )
...
class AuditLogger:
    ...
    def log_user_action(
        self, action: str, user_id: int, target_user_id: Optional[int] = None, details: Optional[dict] = None
    ):
        """
        记录用户操作审计日志
    ...
    def log_data_access(self, action: str, user_id: int, shard_id: str, result: str, details: Optional[dict] = None):
        """
        记录数据访问审计日志
        ...
        """
        self.logger.info(
            f'DATA_ACCESS - {action} - User:{user_id} - Shard:{shard_id} - Result:{result} - Details:{details}'
        )

    def log_permission_change(self, action: str, grantor_id: int, user_id: int, resource_id: str, permissions: dict):
        """
        记录权限变更审计日志
        ...
        """
        self.logger.info(
            f'PERMISSION_CHANGE - {action} - Grantor:{grantor_id} - User:{user_id} - '
            f'Resource:{resource_id} - Permissions:{permissions}'
        )
