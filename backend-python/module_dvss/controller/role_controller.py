...
@router.post("/", response_model=CommonResponse[RoleResponse], summary="创建角色")
...
@router.get("/", response_model=PaginatedResponse[RoleList], summary="获取角色列表")
async def get_roles(
...
@router.get("/{role_id}", response_model=CommonResponse[RoleResponse], summary="获取角色详情")
async def get_role(
    role_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
...
@router.put("/{role_id}", response_model=CommonResponse[RoleResponse], summary="更新角色")
...
@router.delete("/{role_id}", response_model=CommonResponse[None], summary="删除角色")
async def delete_role(
    ...
    if users_count > 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"无法删除角色，仍有 {users_count} 个用户使用此角色"
        )
...
@router.post("/{role_id}/permissions", response_model=CommonResponse[List[RolePermissionResponse]], summary="设置角色字段权限")
...
@router.get("/{role_id}/permissions", response_model=CommonResponse[List[RolePermissionResponse]], summary="获取角色权限")
