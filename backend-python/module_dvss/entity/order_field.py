...
class OrderField(Base):
    ...
    def __repr__(self):
        return f"<OrderField(id={self.id}, field_name='{self.field_name}', sensitivity_level='{self.sensitivity_level}')>"
...
class RoleFieldPermission(Base):
    ...
    field_id = Column(Integer, nullable=False, index=True, comment="字段ID")
    ...
    def __repr__(self):
        return f"<RoleFieldPermission(role_id={self.role_id}, field_id={self.field_id}, can_view={self.can_view}, can_decrypt={self.can_decrypt})>"
