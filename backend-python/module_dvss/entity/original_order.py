...
class OriginalOrder(Base):
    ...
    def __repr__(self):
        return f"<OriginalOrder(id={self.id}, order_id='{self.order_id}', user_id='{self.user_id}', status='{self.status}')>"
    ...
    def get_sensitive_fields(self):
        """获取敏感字段列表"""
        return [
            'name', 'phone', 'email', 'address', 'shipping_address', 'billing_address',
            'payment_info', 'credit_card', 'bank_account', 'total_amount', 'tax_amount'
        ]
