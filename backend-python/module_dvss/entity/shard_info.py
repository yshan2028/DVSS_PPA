...
class ShardInfo(Base):
    ...
    encrypted_order_id = Column(Integer, ForeignKey("encrypted_orders.id"), nullable=False, index=True, comment="加密订单ID")
    ...
    def __repr__(self):
        return f"<ShardInfo(id={self.id}, shard_id='{self.shard_id}', shard_index={self.shard_index}, status='{self.status}')>"
...
class StorageNode(Base):
    ...
    def __repr__(self):
        return f"<StorageNode(id={self.id}, node_name='{self.node_name}', node_type='{self.node_type}', is_active={self.is_active})>"
    ...
    def to_dict(self):
        ...
        return {
            ...
            'capacity_usage_percent': (self.capacity_used / self.capacity_total * 100) if self.capacity_total > 0 else 0,
