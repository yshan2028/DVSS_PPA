...
class UserDao:
    ...
    def create_user(self, user_data: dict) -> DvssUser:
        """
        创建用户
        ...
        try:
            # 检查用户名是否已存在
            existing_user = self.db.query(DvssUser).filter(
                or_(
                    DvssUser.username == user_data['username'],
                    DvssUser.email == user_data['email']
                )
            ).first()
