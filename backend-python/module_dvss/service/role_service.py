...
from module_dvss.schemas.role_schema import RoleCreate, RolePermissionCreate, RoleUpdate
...
class RoleService:
    ...
    async def create_role(self, role_data: RoleCreate) -> Role:
        """创建角色"""
        try:
            role = Role(name=role_data.name, description=role_data.description, is_active=role_data.is_active)
    ...
    async def get_roles_paginated(
        self, page: int = 1, page_size: int = 20, name: Optional[str] = None, is_active: Optional[bool] = None
    ) -> Tuple[List[Role], int]:
        """分页获取角色列表"""
        return await self.role_dao.get_paginated(page=page, page_size=page_size, name=name, is_active=is_active)
    ...
    async def set_role_permissions(
        self, role_id: int, permissions: List[RolePermissionCreate]
    ) -> List[RoleFieldPermission]:
        """设置角色字段权限"""
        try:
            # 删除现有权限
