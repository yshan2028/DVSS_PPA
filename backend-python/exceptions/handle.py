...
async def dvss_exception_handler(request: Request, exc: DVSSException) -> JSONResponse:
    ...
    response_data = ResponseUtil.error(message=exc.message, code=exc.code, data=exc.details)
...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    response_data = ResponseUtil.error(message=exc.detail, code=exc.status_code)
    response_data['timestamp'] = datetime.now().isoformat()
...
async def validation_exception_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
    ...
    response_data = ResponseUtil.error(
        message='请求参数验证失败', code=422, data={'validation_errors': error_details, 'details': exc.errors()}
    )
    response_data['timestamp'] = datetime.now().isoformat()
...
async def general_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    if settings.DEBUG:
        error_detail = {'type': type(exc).__name__, 'message': str(exc), 'traceback': traceback.format_exc()}
    else:
        error_detail = None

    response_data = ResponseUtil.error(message='服务器内部错误', code=500, data=error_detail)
    response_data['timestamp'] = datetime.now().isoformat()
