...
func newGrpcConnection(orgConfig config.OrganizationConfig) (*grpc.ClientConn, error) {
	// 检查是否有TLS证书路径配置
	if orgConfig.TLSCertPath == "" {
		// 没有TLS配置，使用不安全连接
		connection, err := grpc.Dial(orgConfig.PeerEndpoint, grpc.WithTransportCredentials(insecure.NewCredentials()))
		if err != nil {
			return nil, fmt.Errorf("创建非TLS gRPC连接失败：%w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("创建TLS gRPC连接失败：%w", err)
	}
